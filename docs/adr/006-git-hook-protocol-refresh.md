...
```
CLAUDE.md → "re-read warmup.yaml" → full rules restored
    ↑
...
Implement a `asimov refresh` command that outputs protocol reminders, and call it from git pre-commit hooks.
...
```bash
#!/bin/bash
# .hooks/pre-commit (generated by asimov init --asimov)
...
1. **Requires CLI Installation:** `asimov` must be in PATH
...
│                                                                     │
│  Layer 2: Git Hook Refresh (this ADR)                              │
│           └── Fresh output on every commit                          │
│           └── Cannot be compacted (hasn't happened yet)             │
│                                                                     │
